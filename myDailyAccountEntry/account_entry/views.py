...
def sample(request):
    ...
    cash_credit = account_database.objects.filter(date__range=("2023-06-10", "2023-06-13"), credit_debit="credit", account="Cash").aggregate(cash_c=Sum('amount'))
    cash_debit = account_database.objects.filter(date__range=("2023-06-10", "2023-06-13"), credit_debit="debit", account="Cash").aggregate(cash_d=Sum('amount'))
    cash_total_balance = {"cash1": float(cash_credit["cash_c"]), "cash2": float(cash_debit["cash_d"])} 
    balance = (cash_total_balance["cash1"]-cash_total_balance["cash2"])+8000
